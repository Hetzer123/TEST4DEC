...
def hello_world():
    ...
    if request_type_str == 'GET':
        ...
        mysalary = request.form['age']
        mygender = request.form['gender']
        model = load('app/watch-recommender.joblib')
        np_arr = np.array([mysalary, mygender])
        predictions = model.predict([np_arr])  
        predictions_to_str = str(predictions)
        #return predictions_to_str
        return render_template('index.html', href2='The suitable Watch for you (salary:'+str(mysalary)+' ,gender:'+str(mygender)+' ,) is:'+predictions_to_str)
